...
def gardrop_hesap(data):
    ...
    mdflam_adet = math.ceil(
        toplam_mdflam_alan / tabaka_alan
    )
    ...
    arkalik_adet = math.ceil(
        arkalik_alan / tabaka_alan
    )
    ...
    return {
        ...
        "fiyat": satis,
        "musteri_mesaji":
            f"Yaklaşık fiyatımız {satis:,.0f} TL'dir."
    }
...
def mutfak_hesap(data):
    ...
    if alt_metre <= 0 and ust_metre <= 0:
        return {
            "hata": "Alt veya üst dolap metresi gerekli."
        }
    ...
    # Standart yaklaşık mutfak fiyatı
    satis = toplam_metre * 10500

    kapak_turu = str(
        data.get("kapak_turu", "mdflam")
    ).lower()
...
def fiyat_hesapla():
    ...
    if urun in [
        ...
        return jsonify({
            "hata":
                "Bu ürün için otomatik fiyat tanımlı değil.",
